...
{
    public class MPRightShift : MPOperator
    {
        public MPRightShift()
        ...
        public void RightShift(Stack<object> output, object left, object right)
        {
            int result;
            ...
            switch (Type.GetTypeCode(left.GetType()))
            {
                ...
                case TypeCode.UInt32:
                    output.Push((uint)left >> result);
                    break;
                case TypeCode.Int64:
                    output.Push((long)left >> result);
                    break;
                case TypeCode.UInt64:
                    output.Push((ulong)left >> result);
                    break;
                default:
                    throw new InvalidOperatorTypesException(">>", left, right);
            }
